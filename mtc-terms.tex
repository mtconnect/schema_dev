...
\newglossaryentry{probe request}
{
  type=mtc,
  name={Probe Request},
  description={An \glstext{http} request to the \gls{agent} for returning metadata as an \glspl{mtconnectdevice} \glstext{xml} document}
}
...
}
{
  ...
  For example, \cfont{"This is some text"} is the CDATA in the XML element:
...
\longnewglossaryentry{mtconnect document}
{
  type=mtc,
...
\longnewglossaryentry{mtconnect xml document}
{
  type=mtc,
...
\longnewglossaryentry{warning value}
{
  type=mtc,
  category=model,
  name={WARNING},
